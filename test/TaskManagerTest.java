...
import static enumlists.TaskStatus.IN_PROGRESS;
...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    @Test
    void constancyOfTheTask() { //проверка полей задачи
        Task buyingJam = new Task("Купить варенье", "Малиновое", TaskStatus.NEW);
        taskManager.add(buyingJam);
        Map<Integer, Task> taskList = taskManager.getTaskList();
        Task task = taskList.get(1);
    ...
    @Test
    void utilityClassCheck() { //проверка создания таблиц
    ...
    @Test
    void checkingEpicStatus() { //проверка вычисления статуса эпика
        Epic catFood = new Epic("Покормить кота", "Корм для толстых котов и миска", TaskStatus.NEW);
        taskManager.add(catFood);
        taskManager.add(new Subtask("Выбор магазина", "Магазин около дома", TaskStatus.NEW, catFood.getTaskId()));
        taskManager.add(new Subtask("Выбор корма", "Корм для толстых котов", TaskStatus.DONE, catFood.getTaskId()));
        Assertions.assertEquals(IN_PROGRESS, catFood.getTaskStatus(), "Статус рассчитан неверно");
    }
}
